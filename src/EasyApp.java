...
public class EasyApp extends Frame implements ActionListener, ItemListener, MouseListener, KeyListener
{
    ...
    public MenuBar makeMenus(String[] menus)
    {
        for (String menu : menus) {
            if (menu.length() > 0) {
                ...
                Menu newmenu = addMenu(menu.substring(0, d), menubar);

                while (d < menu.length()) {
                    int e = menu.indexOf("|", d + 1);
                    if (e < 0) {
                        e = menu.length();
                    }
                    MenuItem newmi = addMenuItem(menu.substring(d + 1, e), newmenu, this);
    ...
    { byte result=0;
        try{result= Byte.parseByte(input(prompt).trim());}
    ...
    {  long result=0;
        try{result= Long.parseLong(input(prompt).trim());}
        catch (Exception e){result = 0;}
        return result;
    }
    ...
    {  boolean result=false;
        try{result= Boolean.parseBoolean(
                input(prompt).trim());}
